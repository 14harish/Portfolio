...
function Navbar() {
  ...
  return (
    <div className='sm:fixed flex sm:flex-col sm:gap-4 gap-2 sm:top-64 sm:right-24 sm:w-10 mt-10 ml-10 sm:mt-0 pb-4'>
      <button 
      className={`group flex items-center justify-start sm:w-11 sm:h-11 w-14 h-14 dark:bg-${isActive("/Portfolio")  ? "[#FFB400]" : "[#2B2A2A]"} bg-${isActive("/Portfolio") ? "[#FFB400]" : "[#EEEEEE]"} dark:bg-${isActive("/Portfolio/")  ? "[#FFB400]" : "[#2B2A2A]"} bg-${isActive("/Portfolio/") ? "[#FFB400]" : "[#EEEEEE]"} text-[#666666] dark:text-white rounded-full cursor-pointer relative overflow-hidden transition-all duration-300 shadow-lg hover:w-32 active:translate-x-1 active:translate-y-1 hover:bg-[#FFB400] dark:hover:bg-[#FFB400] hover:text-white`}
